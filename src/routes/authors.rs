...
pub async fn handle_request_specific(_req: Request, ctx: RouteContext<()>) -> Result<Response> {
    ...
    let json: Value = serde_json::from_str(data::AUTHORS).unwrap();
    let json =  match json["authors"].as_array() {
        Some(val) => val,
        None => {
            return Response::error("Invalid authors data", 500)
        }
    };

    for val in json {
        if val["_id"].as_str().unwrap() == id.as_str() || val["name"].as_str().unwrap() == id.as_str() || val["slug"].as_str().unwrap() == id.as_str() {
            return Response::ok(val.to_string());
        }
    }

    Response::error("Author does not exist", 404)
}
